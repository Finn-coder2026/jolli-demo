...
export async function saveActiveConvoId(convoId: number | undefined): Promise<void> {
	await mkdir(CONFIG_DIR, { recursive: true });
	...
	if (convoId !== undefined) {
		config.activeConvoId = convoId;
	}
	await writeFile(CONFIG_FILE, JSON.stringify(config, undefined, 2), "utf-8");
}

export async function loadActiveConvoId(): Promise<number | undefined> {
	try {
		const content = await readFile(CONFIG_FILE, "utf-8");
		const config: Config = JSON.parse(content);
		return config.activeConvoId;
	} catch {
		return;
	}
}
