 ...
 */
/**
 * Re-export branding types from jolli-common
 */
export type {
	ApiEndpoint,
	ApiInfo,
...
export interface ThemeConfig {
	...
	logoUrl?: string;
	/** How the logo is displayed: text only, image only, or both (default: text) */
	logoDisplay?: "text" | "image" | "both";
	/** Favicon URL (defaults to generated "D" icon) */
	favicon?: string;
	/** Primary accent color hue 0-360 (default: 212 blue) */
	primaryHue?: number;
	/** Custom footer text - prepended to "Powered by Jolli" @deprecated Use footerConfig instead */
	footer?: string;
	/** Initial theme for visitors (default: 'system') */
	defaultTheme?: DefaultThemeType;
	/** GitHub or project repository link (defaults to Jolli-sample-repos/{siteName}) @deprecated Removed from UI */
	projectLink?: string;
	/** Documentation repository base URL */
	docsRepositoryBase?: string;
	/** Discord/Slack community URL (no icon shown if not set) @deprecated Use headerLinks instead */
	chatLink?: string;
	/** Icon for chat link (default: 'discord', only used if chatLink is set) @deprecated Use headerLinks instead */
	...
	/** Header external links configuration (dropdown or individual links) */
	headerLinks?: import("jolli-common").HeaderLinksConfig;
	/** Footer configuration (copyright, columns with links) */
	footerConfig?: import("jolli-common").FooterConfig;
	/** Font family for typography */
	fontFamily?: import("jolli-common").FontFamily;
	/** Syntax highlighting theme for code blocks */
	codeTheme?: import("jolli-common").CodeTheme;
	/** Corner roundness for UI elements */
	borderRadius?: import("jolli-common").BorderRadius;
	/** Whitespace density between elements */
	spacingDensity?: import("jolli-common").SpacingDensity;
	...
	pageWidth?: import("jolli-common").PageWidth;
	/** Article content area max-width for readability (default: "standard") */
	contentWidth?: import("jolli-common").ContentWidth;
	/** Left sidebar navigation panel width (default: "standard") */
	sidebarWidth?: import("jolli-common").SidebarWidth;
	/** Right table-of-contents panel width (default: "standard") */
	tocWidth?: import("jolli-common").TocWidth;
	/** Header navigation link alignment (default: "right") */
	headerAlignment?: import("jolli-common").HeaderAlignment;
}
...
/**
 * API page metadata for internal page links (OpenAPI docs within Nextra)
 * Used in _meta.ts for linking to standalone API documentation pages
 * Uses type: 'page' which maps to internal files
 *
...
export interface ApiPageMeta {
	/** Display title in navigation */
	title: string;
	/** Page type - must be 'page' for internal pages */
	type: "page";
	/** URL to the API docs page (internal path). Optional for navbar tabs. */
	href?: string;
}

/**
 * External link metadata for navbar items pointing to external URLs
 * Used in _meta.ts for pure external links (no type: 'page' needed)
 * Per Nextra docs, external links just need href without type
 * Note: Nextra 4's strict schema doesn't support newWindow property
 */
...
export interface ArticleInput {
	...
	content: string;
	/** Content type (e.g., "text/markdown", "text/mdx", "application/json", "application/yaml") */
	contentType?: string;
	...
	/**
	 * Folder path within the site content structure.
	 * Empty string means root level.
	 * Examples: "getting-started", "guides/advanced"
	 * Used to preserve Jolli's folder hierarchy when generating Nextra sites.
	 */
	folderPath?: string;
	/**
	 * The document's actual slug from the database.
	 * This is the unique slug with any suffixes, used for file naming.
	 * If not provided, slug is derived from title.
	 */
	slug?: string;
	/**
	 * Whether this document is a folder (has children).
	 * Folder documents become index.md inside their folder.
	 */
	isFolder?: boolean;
}
...
export interface GenerateToMemoryOptions {
	...
	auth?: AuthConfig;
	/** Regeneration mode - accepted for API compatibility but no longer changes output (all files are always generated) */
	regenerationMode?: boolean;
	/** Theme configuration overrides */
	theme?: Partial<ThemeConfig>;
	/** Migration mode - accepted for API compatibility (previously overrode regenerationMode) */
	migrationMode?: boolean;
	/** Migration context with existing files and parsed config (for 3.x to 4.x migration) */
	migrationContext?: MigrationContext;
	/** When true, preserve existing _meta.ts ordering instead of re-sorting to match space order.
	 *  Used when auto-sync is off (manual article selection) so user nav customizations survive publish. */
	preserveNavOrder?: boolean;
	/** When true, force article folder paths to match the space tree structure.
	 *  Overrides any manual file moves in the repo so articles return to their space-derived folders. */
