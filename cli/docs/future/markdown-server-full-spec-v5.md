...
- v3 spec fully implemented (commit history - needed for cleanup considerations)
...
### Commit History Cleanup (v3 consideration)

When deleting tombstones, also consider cleaning up associated commit history:

```typescript
// After deleting tombstone doc
await docDao.deleteDoc(doc.jrn);

// Option: Also delete commit files for this doc
...
// Then clean up orphaned commits (commits with no files)
await sequelize.query(`
...
This is optional - commit history for deleted files may still be valuable for audit purposes.
