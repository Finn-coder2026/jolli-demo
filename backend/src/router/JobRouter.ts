...
export function createJobRouter(
	...
	// Handle both old signature (scheduler, daoProvider, tokenUtil) and new config object
	...
	if ("jobDaoProvider" in jobSchedulerOrConfig) {
		...
		if (!jobDaoProvider || !tokenUtil) {
			throw new Error("jobDaoProvider and tokenUtil are required when using old signature");
		}
	 ...
	 */
	router.get("/dashboard-active", async (_req: Request, res: Response) => {
		try {
			const jobDao = daoProvider.getDao(getTenantContext());
			const scheduler = await getSchedulerForRequest();
			const activeJobs = await jobDao.listJobExecutions({
	 ...
	 */
	router.post("/:id/unpin", async (req: Request, res: Response) => {
		try {
			const jobDao = daoProvider.getDao(getTenantContext());
			const jobId = req.params.id;
			const userId = getUserId(req, tokenUtilResolved);
